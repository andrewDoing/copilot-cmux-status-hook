export function createCmuxSurfaceApplicator({ config, cmux }) {
  ...
  async function clearWorkspaceDescription({ force = false } = {}) {
    if (!config.workspaceCard) return;
    if (!force && applied.workspaceDescription === undefined) return;
    if (applied.workspaceDescription === null) return;
    applied.workspaceDescription = null;
    await cmux(["workspace-action", "--action", "clear-description"]);
  }
  ...
  return {
    ...
    async clearOwnedSurfaces({ forceWorkspaceDescription = false } = {}) {
      await applyStatus(null);
      if (config.progressBar) await applyClearProgress();
      await clearWorkspaceDescription({ force: forceWorkspaceDescription });
    },

    async clearStartupSurfaces() {
      for (const key of startupStatusKeys(config.statusKey, config.clearLegacyStatusKeys)) {
        await cmux(["clear-status", key]);
      }
      applied.status = null;
      await clearWorkspaceDescription({ force: true });
      if (config.clearLogOnStartup) await cmux(["clear-log"]);
    },
  };
}

function startupStatusKeys(statusKey, clearLegacyStatusKeys = false) {
  const keys = [statusKey];
  if (clearLegacyStatusKeys) keys.push("copilot-cli", "copilot");
