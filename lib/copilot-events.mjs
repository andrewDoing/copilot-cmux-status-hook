...
export function registerCmuxStatusEvents(session, controller) {
  ...
  on("user.message", (event) => {
    ...
    void controller.skillContextMessage(event.data.content);
    void controller.goalModeMessage(event.data.content);
  });

  on("system.message", (event) => {
    void controller.skillContextMessage(event.data.content);
    void controller.goalModeMessage(event.data.content);
  });

  on("system.notification", (event) => {
    void controller.skillContextMessage(event.data.content);
    void controller.goalModeMessage(event.data.content);
  });
  ...
  on("skill.invoked", (event) => {
    void controller.skillInvoked(event.data);
  });
  ...
  on("assistant.usage", (event) => {
    void controller.assistantUsage(event.data);
  });
  ...
  on("session.shutdown", (event) => {
    ...
    void controller.clearStatus().finally(() => controller.dispose());
  });
}
