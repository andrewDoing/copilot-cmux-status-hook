...
export function createCmuxStatusController(options = {}) {
  ...
  const config = {
    ...
    dedupeSidebar: booleanFromEnv(env.CMUX_COPILOT_DEDUPE_SIDEBAR, DEFAULTS.dedupeSidebar),
  ...
  async function setWorkspaceCard(description, color) {
    ...
    const dedupedDescription = await dedupeWithSidebar(description);
    await cmux(["workspace-action", "--action", "set-description", "--description", dedupedDescription]);
    await cmux(["workspace-action", "--action", "set-color", "--color", color]);
  }

  async function dedupeWithSidebar(description) {
    if (!config.dedupeSidebar) return description;
    const labels = [lastStatusLabel, lastProgressLabel, lastWorkspaceTitleLabel];
    labels.push(...sidebarStatusLabels(await cmuxOutput(["sidebar-state"])));
    return dedupeWorkspaceLines([description], labels, { keepContext: true });
  }
  ...
  async function setWorkspaceTitle(badge, colorName) {
    ...
    if (!title) return;
    lastWorkspaceTitleLabel = `${badge} ${title}`;
    await cmux(["workspace-action", "--action", "rename", "--title", lastWorkspaceTitleLabel]);
  ...
  function cardDescription(label) {
    const lines = [];
    if (config.showPermissions && permissionActive && permissionMessage && permissionMessage !== label) {
      lines.push(permissionMessage);
    }
    if (config.showSubagents && activeSubagents.size > 0) lines.push(formatSubagentSummary(activeSubagents));
    const turnActivity = turnActivitySummary(turnStats, config);
    if (turnActivity) lines.push(turnActivity);
    const skills = config.showSkills ? skillSummary(turnStats.skills) : "";
    if (skills) lines.push(skills);
    if (config.showContext && contextUsage) lines.push(`${contextSeverity().badge} ${contextUsage.label}`);
    if (config.showAic && aiCreditsUsed > 0) lines.push(`💳 AIC used: ${formatAic(aiCreditsUsed)}`);
    const compaction = config.showCompactions ? compactionLabel(compactionActive, compactionCount) : "";
    if (compaction) lines.push(compaction);
    const elapsed = elapsedLabel();
    if (config.showElapsed && state === "working" && elapsed) lines.push(elapsed);
    return dedupeWorkspaceLines(lines, [label, lastStatusLabel, lastProgressLabel, lastWorkspaceTitleLabel], {
      keepContext: true,
    });
  }
  ...
  return {
    ...
    async done({ aborted = false } = {}) {
      ...
      if (!aborted && !attentionActive && config.notifyOnDone) {
        await notify("Copilot is done", cardDescription(label) || label);
      }
    ...
    async assistantUsage(data = {}) {
      ...
      aiCreditsUsed += cost;
      await updateVisibleState(currentStatusLabel({
        attentionActive,
        attentionMessage,
        compactionCount,
        currentActivity,
      ...
      await log("info", `AIC used: ${formatAic(aiCreditsUsed)} (+${formatAic(cost)} ${data?.model || "model"})`);
    },
...
function dedupeWorkspaceLines(lines, statusLabels, { keepContext = false } = {}) {
  ...
  for (const group of lines) {
    for (const line of String(group || "").split("\n")) {
      ...
      if (!(keepContext && isContextLine(line)) && keys.some((key) => seen.has(key))) continue;
...
function sidebarStatusLabels(output) {
  const labels = [];
  for (const line of String(output || "").split("\n")) {
    const match = line.match(/^\s{2}[^=\s]+=(.*?)\s+icon=/);
    if (match?.[1]) labels.push(match[1]);
  }
  return labels;
}

function isContextLine(value) {
  return /\bcontext\s+\d+%/i.test(String(value || ""));
}
