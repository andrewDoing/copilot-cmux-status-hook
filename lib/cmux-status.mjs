...
export function createCmuxStatusController(options = {}) {
  ...
  async function setStatus(key, value, icon, color, priority) {
    return cmux(["set-status", key, value, "--icon", icon, "--color", color, "--priority", String(priority)]);
  }
...
export function contextStatusValue(usage) {
  const percent = Math.round(usage.ratio * 100);
  const messages = usage.messagesLength === undefined ? "" : `, ${usage.messagesLength} msgs`;
  return `Context ${percent}% (${formatTokenCount(usage.currentTokens)}/${formatTokenCount(usage.tokenLimit)}${messages})`;
}
