...
export const DEFAULTS = {
  ...
  workspaceColor: true,
  workspaceDoneColor: "Green",
  workspaceWaitingColor: "Red",
  workspaceWorkingColor: "Amber",
};

const TERMINAL_EMOJIS = ["🦊", "🐙", "🦉", "🐝", "🐢", "🦀", "🐬", "🦄", "🚀"];
const WORKING_MARKER = "Working";
const WAITING_MARKER = "Waiting on user";
...
export function createCmuxStatusController(options = {}) {
  ...
  const config = {
    ...
    storeDir: env.CMUX_COPILOT_STORE_DIR || DEFAULTS.storeDir,
    workspaceColor: booleanFromEnv(env.CMUX_COPILOT_WORKSPACE_COLOR, DEFAULTS.workspaceColor),
    workspaceDoneColor: env.CMUX_COPILOT_WORKSPACE_DONE_COLOR || DEFAULTS.workspaceDoneColor,
    workspaceWaitingColor: env.CMUX_COPILOT_WORKSPACE_WAITING_COLOR || DEFAULTS.workspaceWaitingColor,
    workspaceWorkingColor: env.CMUX_COPILOT_WORKSPACE_WORKING_COLOR || DEFAULTS.workspaceWorkingColor,
    ...options,
  };
  ...
  async function setActivity(value, detail) {
    await setWorkspaceLifecycleColor("working");
    currentActivityMarker = WORKING_MARKER;
    currentActivity = detail;
    await setActivityProgress(value, detail);
    await renderContextStatus();
  }

  async function setWaitingActivity() {
    await setWorkspaceLifecycleColor("waiting");
    currentActivityMarker = WAITING_MARKER;
    currentActivity = "input needed";
    await renderContextStatus();
  }

  async function setWorkspaceLifecycleColor(state) {
    if (!config.workspaceColor) return false;
    const color = workspaceLifecycleColor(state, config);
    if (!color || workspaceColorApplied === color) return false;
    if (await cmux(["workspace-action", "--action", "set-color", "--color", color])) {
      workspaceColorApplied = color;
      return true;
    }
    return false;
  }
  ...
  async function renderContextStatus() {
    ...
    contextStatusApplied = await setStatus(
      ...
      `${contextIcon(latestContextUsage, config)} ${contextStatusValue(latestContextUsage, await getTerminalPreviewLabel(), currentActivity, currentActivityMarker)}`,
      "",
  ...
  async function clearStaleStartupSurfaces() {
    ...
    const keys = new Set(["copilot-aic"]);
  ...
  return {
    ...
    async toolStart(toolName, args) {
      if (isWaitingOnUserTool(toolName)) {
        await setWaitingActivity();
        return;
      }
    ...
    async clearStatus() {
      await setWorkspaceLifecycleColor("done");
...
export async function assignTerminalOrdinal(filePath, terminalKey) {
  ...
  await appendJsonLine(filePath, { terminalKey });
...
export function contextStatusValue(usage, terminalLabel = "Copilot", activity = "", activityMarker = WORKING_MARKER) {
  ...
  const prefix = activity ? `${activityMarker}: ${activity} · ${context}` : context;
...
export function workspaceLifecycleColor(state, config = DEFAULTS) {
  if (state === "working") return config.workspaceWorkingColor;
  if (state === "waiting") return config.workspaceWaitingColor;
  if (state === "done") return config.workspaceDoneColor;
  return "";
}
...
function isWaitingOnUserTool(toolName) {
  return /\b(ask[_-]?user|elicitation|user[_-]?input)\b/i.test(String(toolName || ""));
}
