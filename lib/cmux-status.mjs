...
export function createCmuxStatusController(options = {}) {
  ...
  function cardDescription(label) {
    ...
    const turnActivity = turnActivitySummary(turnStats);
    if (turnActivity) lines.push(turnActivity);
    const skills = skillSummary(turnStats.skills);
    if (skills) lines.push(skills);
    if (contextUsage) lines.push(`${contextSeverity().badge} ${contextUsage.label}`);
    if (aiCreditsUsed > 0) lines.push(`💳 AIC used: ${formatAic(aiCreditsUsed)}`);
...
function turnActivitySummary(turnStats) {
  const parts = [];
  if (turnStats.toolCount > 0) parts.push(`🛠 Tools invoked: ${turnStats.toolCount}`);
...
function skillSummary(skills) {
  ...
  if (names.length === 0) return "";
  return `🧰 Skills: ${names.slice(0, 3).join(", ")}${names.length > 3 ? ", ..." : ""}`;
}
