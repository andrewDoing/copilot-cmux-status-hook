...
export const DEFAULTS = {
  ...
  clearOnStartup: true,
  clearLegacyStatusKeys: false,
  clearStatusOnDispose: true,
...
export function createCmuxStatusController(options = {}) {
  ...
  const config = {
    ...DEFAULTS,
    statusKey: env.CMUX_COPILOT_STATUS_KEY || defaultStatusKey(env),
    ...
    progressBar: booleanFromEnv(env.CMUX_COPILOT_PROGRESS_BAR, DEFAULTS.progressBar),
    ...
    clearLegacyStatusKeys: booleanFromEnv(env.CMUX_COPILOT_CLEAR_LEGACY_STATUS_KEYS, DEFAULTS.clearLegacyStatusKeys),
    clearStatusOnDispose: booleanFromEnv(env.CMUX_COPILOT_CLEAR_STATUS_ON_DISPOSE, DEFAULTS.clearStatusOnDispose),
  ...
  function startPulse() {
    stopPulse();
    if (!config.progressBar || config.pulseIntervalMs <= 0) return;
    pulseTimer = timers.setInterval(() => {
      ...
      void renderVisibleState(label, { progress: { value: progress } });
  ...
  function resetTurnDetails() {
    activeSubagents = new Map();
    aiCreditsUsed = 0;
    usageCallIds = new Set();
  ...
  async function applyWorking(detail, { reset = false, progressLabel } = {}) {
    ...
    if (!plan.contextProgressActive) startPulse();
  }
  ...
  return {
    ...
    async done({ aborted = false } = {}) {
      ...
      stopElapsedTimer();
      activeSubagents = new Map();
      if (!attentionActive) resetTurnDetails();
      ...
      if (config.progressBar && !plan.contextProgressActive) clearProgressLater();
    },
    ...
    async clearStatus() {
      await surfaces.clearOwnedSurfaces();
    },
    ...
    dispose() {
      ...
      if (config.clearStatusOnDispose) void this.clearStatus();
    },
  };
}

function defaultStatusKey(env) {
  return `copilot-${env.CMUX_SURFACE_ID || process.pid}`;
}
