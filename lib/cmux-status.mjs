...
export function createCmuxStatusController(options = {}) {
  ...
  function cardDescription(label) {
    return contextUsage ? `${label}\n${contextUsage.label}` : label;
  }
  ...
  return {
    ...
    async ready(label = "Copilot ready") {
      ...
      await setWorkspaceCard(cardDescription(label), "Green");
    ...
    async startWorking(detail = "working") {
      ...
      await setWorkspaceCard(cardDescription(label), "Amber");
    ...
    async toolComplete(toolName, success) {
      ...
      if (!success) {
        ...
        await setWorkspaceCard(cardDescription(`Copilot working: ${name} failed`), "Red");
      }
    },
    ...
    async done({ aborted = false } = {}) {
      ...
      await setWorkspaceCard(cardDescription(label), aborted ? "Charcoal" : "Green");
    ...
    async error(message) {
      ...
      await setWorkspaceCard(cardDescription(label), "Red");
    ...
    async contextUsage(data) {
      ...
      await renderContextProgress();
      if (state === "working") {
        await setWorkspaceCard(cardDescription("Copilot working"), "Amber");
      } else if (state === "error") {
        await setWorkspaceCard(cardDescription("Copilot needs attention"), "Red");
      } else {
        await setWorkspaceCard(cardDescription("Copilot done - waiting"), "Green");
      }
    },
