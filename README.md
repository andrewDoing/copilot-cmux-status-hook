...
- **Working:** `🤖` workspace title, amber workspace sidebar, and a labeled pulsing progress bar until context usage is known; optional workspace-card elapsed details when explicitly enabled.
...
- **Tool activity:** tracks tool counts internally without adding rows that duplicate CMUX native Copilot details.
- **AIC usage:** tracks `assistant.usage` billing cost internally without adding rows that duplicate CMUX native Copilot details.
- **Goal mode:** tracks injected `/autopilot` objectives for optional workspace-card details.
- **Skills:** tracks `skill.invoked` and injected `<skill-context name="...">` blocks, then shows invoked skill names in optional workspace-card details.
...
Each CMUX surface has one owner: CMUX's native Copilot hooks own lifecycle status rows, while this hook keeps the workspace title/color and supplemental progress/log/notification details fresh by default. Optional workspace-card details can be re-enabled with `CMUX_COPILOT_WORKSPACE_CARD=1`; when disabled, startup still clears stale card descriptions left by older versions of this hook. If context progress is disabled and workspace-card details are enabled, the workspace card shows context instead. The hook clears stale status entries that older versions created, builds a render plan for each event, then applies only changed CMUX surfaces so unchanged values are not re-emitted.
...
| `CMUX_COPILOT_WORKSPACE_CARD` | `0` | Set to `1` to update the visible CMUX workspace card description/color with supplemental details. Leave disabled when using CMUX native Copilot hooks. |
...
| `CMUX_COPILOT_SHOW_AIC` | `1` | Compatibility setting; AIC is tracked internally and left to CMUX native Copilot details instead of workspace-card rows. |
...
| `CMUX_COPILOT_SHOW_TOOL_ACTIVITY` | `1` | Compatibility setting; tool activity is logged and left to CMUX native Copilot details instead of workspace-card rows. |
